...
read_terraform_outputs() {
  ...
  server_ips="$(
    printf '%s\n' "${terraform_output}" |
      jq -r '.nomad_server_private_ips.value.[]'
  )"
  client_asg_name="$(
    printf '%s\n' "${terraform_output}" |
  ...
  if [ -z "${instance_ids}" ]; then
    log "ERROR: No InService instances found in ASG:" "${client_asg_name}"
    exit 1
  fi
  ...
  log "  Nomad server nodes:" "$(printf '%s\n' "${server_ips}" | tr '\n' ' ')"
  log "  Nomad client nodes:" "$(printf '%s\n' "${client_ips}" | tr '\n' ' ')"
